...
//	}
	...
	public void buy(UserVO user, ProdVO prod) {
		int no=0;
		while (true) {
			System.out.println("연락처와 주소를 확인해 주세요.");
			System.out.println("연락처: " + user.getPhone() + "  주소: " + user.getAddr());
			System.out.println("1.다음으로 2.연락처/주소 변경 0.돌아가기");
			System.out.print("> ");
			no = scn.nextInt();
			scn.nextLine();
			if(no==1) {
	...
	private void writeReview(UserVO user) {
		...
		if (!dao.rc(user, no)) {
			System.out.println("해당 구매내역 없거나 미발송 상태입니다.");
			return;
		}
