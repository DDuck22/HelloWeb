...
public class MallDao {
	...
	public boolean removeProd(int no) {
		sql = "update mall_product set show_p = 0 where prod_no = ?";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
	...
	public List<ProdVO> stock() {
		...
		sql = "select * from mall_product where show_p = 1 order by prod_no";
		conn = Dao.getConnect();
	...
	public int manageStock(int no) {
		sql = "select * from mall_product where prod_no = ? and show_p = 1";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setInt(1, no);
	...
	public List<ProdVO> list() {
		List<ProdVO> list = new ArrayList<>();
		sql = "select * from mall_product where show_p = 1 order by click_cnt desc";
		conn = Dao.getConnect();
	...
	public ProdVO searchName(String str) {
		sql = "select * from mall_product where prod_name = ? and show_p = 1";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
	...
	public List<OrderVO> buyHistory(String id) {
		...
		sql = "select * from buy_history join mall_product using (prod_no) where user_id = ? order by buy_no";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, id);
			...
			while (rs.next()) {
				...
				order.setTotalPrice(rs.getInt("total_price"));
				order.setDate(rs.getString("buy_date"));
				order.setOrderCheck(rs.getInt("order_check"));
	...
	public boolean rc(UserVO user, int no) {
		sql = "select * from buy_history where user_id = ? and buy_no = ? and order_check = 1";
		conn=Dao.getConnect();
		try {
			psmt=conn.prepareStatement(sql);
	...
	public boolean removeReview(UserVO user, int no) {
		sql = "update buy_history set review = null, review_date = null " //
				+ " where user_id = ? and buy_no = ? and review is not null";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, user.getId());
